...
SELECT *
    FROM PROFESSOR
    WHERE HIREDATE BETWEEN TO_DATE('1994-01-01 00:00:00') 
        AND TO_DATE('1998-02-02 23:59:59')
    ORDER BY HIREDATE;
...
SELECT *
    ...
    WHERE HIREDATE BETWEEN 
                TO_DATE('19940101 00:00:00', 'yyyyMMdd HH24:mi:ss')
                    AND TO_DATE('19980202 23:59:59', 'yyyyMMdd HH24:mi:ss') --2월 3일은 포함되면 안된다.
    ORDER BY HIREDATE;
...
SELECT *
 ...
 --별칭은 큰따옴표가 가능하다. AS 생략 가능.
SELECT ENO AS "직원번호"
    , ENAME "직원이름"
    FROM EMP
    WHERE JOB IN ('개발', '경영') --쿼리 안에서 사용할 때는 작은 따옴표 써라.
        AND COMM >= 700;
...
SELECT A.* --A가 조회하는 테이블로 기준이됨.
    , B.PNAME
    FROM COURSE A
--    INNER JOIN PROFESSOR B --이렇게 하면, 교집합에서 null인 경우는 매핑이 안된다.
    LEFT OUTER JOIN PROFESSOR B --여기는 교집합데이터와 남은거 다 출력시킨다!
    ON A.PNO = B.PNO;
